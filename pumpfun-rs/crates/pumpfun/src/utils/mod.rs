...
pub async fn create_token_metadata_new(
    metadata: CreateTokenMetadata,
) -> Result<TokenMetadataResponse, Box<dyn std::error::Error>> {
    let mut file = File::open(metadata.file.as_str())?;
    let mut file_contents = Vec::new();
    file.read_to_end(&mut file_contents)?;
    ...
    let metadata_uri = metadata_json["metadataUri"]
        .as_str()
        .ok_or("Invalid metadata URI")?;

    // Construct the response
    let response = TokenMetadataResponse {
        metadata: TokenMetadata {
            name: metadata.name,
            symbol: metadata.symbol,
            description: metadata.description,
            image: metadata_uri.to_string(),
            show_name: true,
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_create_token_metadata_new() {
        let file_path = "/tmp/cat2.png";

        // Create test metadata
        let metadata = CreateTokenMetadata {
            name: "cat2".to_string(),
            symbol: "cat2".to_string(),
            description: "cat2 Description".to_string(),
            file: file_path.to_string(),
            twitter: Some("https://x.com/test_twitter".to_string()),
            telegram: Some("https://telegram.org/test_telegram".to_string()),
            website: Some("https://example.com".to_string()),
        };

        let result = create_token_metadata_new(metadata).await;
        assert!(result.is_ok());

        let response = result.unwrap();
        assert_eq!(response.metadata.name, "cat2");
        assert_eq!(response.metadata.symbol, "cat2");
        assert_eq!(response.metadata.description, "cat2 Description");
        assert!(response.metadata.image.starts_with("https://ipfs.io/ipfs/"));
        assert!(response.metadata.show_name);
        assert_eq!(response.metadata.created_on, "https://pump.fun");
        assert!(response.metadata_uri.starts_with("https://ipfs.io/ipfs/"));
