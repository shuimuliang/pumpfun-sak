...
impl PumpFun {
    ...
    pub fn new(
        ...
        let client: Client<Arc<Keypair>> = if let Some(options) = options {
            Client::new_with_options(cluster.clone(), Arc::new(payer.insecure_clone()), options)
        } else {
            Client::new(cluster.clone(), Arc::new(payer.insecure_clone()))
        };
    ...
    pub async fn create(
        ...
        let ipfs: utils::TokenMetadataResponse =
            utils::create_token_metadata_new(metadata)
                .await
    ...
    pub async fn buy(
        ...
        // Get accounts and calculate buy amounts
        let global_account = self.get_global_account()?;
        println!("get_global_account took: {}", start.elapsed().as_secs_f64());
        ...
        let buy_amount_with_slippage =
            utils::calculate_with_slippage_buy(amount_sol, slippage_basis_points.unwrap_or(500));
        println!(
            "get_bonding_curve_account took: {}",
        ...
        println!(
            "create_associated_token_account_idempotent took: {}",
        ...
        println!("buy took: {}", start.elapsed().as_secs_f64());
        Ok(instructions)
    }
    ...
    pub async fn sell(
        ...
        if close_token_ata {
            instructions.push(
                ...
                .map_err(|_| error::ClientError::CloseAccountError)?,
            );
        };
